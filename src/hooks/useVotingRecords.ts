...
export function useVotingRecords(legislatorName: string | undefined, chamber?: string, jurisdiction?: string, bioguideId?: string, level?: string) {
  return useQuery<VotingRecordsResponse>({
    queryKey: ["voting-records", legislatorName, chamber, jurisdiction, bioguideId, level],
    queryFn: async () => {
      ...
      const { data, error } = await supabase.functions.invoke("fetch-voting-records", {
        body: { legislatorName, chamber, jurisdiction, bioguideId, level },
      });
